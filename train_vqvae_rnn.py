...
from utils.misc import LSIZE, RSIZE, RED_SIZE, SIZE, IncreaseSize, N_COLOR_DIM
...
mdrnn = RNNCat(latent_shape, code_dim, K, action_dim, RSIZE).to(device)
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    for i, data in enumerate(loader):
        ...
        if discrete:
            tmp = torch.zeros(*action.size(), action_dim).to(device)
            tmp.scatter_(2, action.long().unsqueeze(2), 1)
            action = tmp
...
for e in range(epochs):
    ...
    if is_best:
        cur_best = test_loss
    tmp = RNNCatCell(latent_shape, code_dim, K, action_dim, RSIZE)
